...
public class CartaoDeVacinaService {
    ...
    @Transactional
    public CartaoDeVacinaResponse atualizar(Long id, CartaoDeVacinaRequest request) {
        CartaoDeVacina cartaoExistente = cartaoDeVacinaRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Cartão de vacina não encontrado"));

        Pet pet = petRepository.findById(request.getPetId())
                .orElseThrow(() -> new RuntimeException("Pet não encontrado"));

        cartaoExistente.setNome(request.getNome());
        cartaoExistente.setPet(pet);

        CartaoDeVacina cartaoAtualizado = cartaoDeVacinaRepository.save(cartaoExistente);
    ...
    public void deletar(Long id) {
        CartaoDeVacina cartao = cartaoDeVacinaRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Cartão de vacina não encontrado"));
        cartaoDeVacinaRepository.delete(cartao);
    }
